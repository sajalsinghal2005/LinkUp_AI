...
function ResumeBuilder() {
  ...
  const tuneBullet = async () => {
    ...
    setTuning(true);
    setImprovedBullet("");
    const toastId = toast.loading("🤖 Refining your bullet point...");

    const prompt = `
You are a professional ATS resume writer. Optimize and rewrite the following weak resume bullet point.

Rules:
1. Apply the Action Verb + Result + Technical Context formula.
2. Use strong, active, professional verbs (e.g. Architected, Orchestrated, Engineered, Spearheaded).
3. If no numbers/percentages are provided, insert realistic metrics/estimates (e.g. "by 18%", "saving 4 hours/week") to make it quantifiable, but do not make it sound fake or bloated.
4. Return ONLY the improved bullet point as plain text. No bullet symbols, no quotes, no chatty introductions, no extra sentences. Just the single polished bullet point.
...
`;
    ...
    try {
      ...
      setImprovedBullet(response.text?.replace(/^[-*•]\s*/, "").replace(/^"+|"+$/g, "").trim() || "");
      toast.success("Bullet point optimized!", { id: toastId });
    } catch (err) {
      console.error(err);
      toast.error("Failed to tune bullet point.", { id: toastId });
    } finally {
      setTuning(false);
    }
  };
...
`;
  ...
  return (
    <div className="flex min-h-screen bg-[#07080d] pt-16 lg:pt-0 w-full overflow-x-hidden font-sans">
      ...
      <div className="flex-1 w-full overflow-x-hidden flex flex-col relative z-10 print:p-0 print:bg-white print:text-black">
        <div className="print:hidden">
          <Navbar userData={null} />
        </div>

        <div className="p-4 sm:p-6 lg:p-8 flex-1 space-y-8 animate-fade-in-up print:p-0">
          ...
          <div className="print:hidden">
            ...
            <p className="mt-2 text-sm text-[#94a3b8] font-medium">
              Fill in your details, generate AI optimized summaries, and print or export your ATS-compliant resume sheet.
            </p>
          </div>
          ...
          <div className="grid gap-8 grid-cols-1 xl:grid-cols-2">
            ...
            <div className="flex flex-col gap-6 print:hidden">
              <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-white/10 space-y-6">
                <h2 className="text-xl font-bold text-[#22d3ee] font-display">Resume Details</h2>
                ...
                <div className="grid gap-4 text-xs font-semibold">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <input
                      ...
                      className="rounded-2xl border border-white/10 bg-white/5 p-4 text-white outline-none focus:border-[#22d3ee] transition-all"
                    />
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-slate-300 mb-2">Technical Skills (e.g. Languages: Java, Python)</label>
                    <textarea
                      placeholder="Languages: C, C++, Java, JavaScript&#10;Frontend: HTML5, CSS3, React&#10;Backend: Node.js, Express.js"
                      value={skills}
                      onChange={(e) => setSkills(e.target.value)}
                      className="w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-white outline-none focus:border-[#22d3ee] min-h-[100px] font-medium leading-relaxed"
                    />
                  </div>

                  <div>
                    <label className="block text-slate-300 mb-2">Education (e.g. B.Tech | SKIT, Jaipur | 2024 - 2028)</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-slate-300 mb-2">Projects</label>
                    <textarea
                      ...
                      className="w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-white outline-none focus:border-[#22d3ee] min-h-[120px] font-medium leading-relaxed"
                    />
                  </div>

                  <div>
                    <label className="block text-slate-300 mb-2">Work Experience</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-slate-300 mb-2">Certifications</label>
                    <textarea
                      placeholder="Java Basic & Advanced | E&ICT Academy, IIT Kanpur&#10;Python for Data Science | E&ICT Academy, IIT Kanpur"
                      value={certifications}
                      onChange={(e) => setCertifications(e.target.value)}
                      className="w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-white outline-none focus:border-[#22d3ee] min-h-[80px] font-medium leading-relaxed"
                    />
                  </div>

                  <div>
                    <label className="block text-slate-300 mb-2">Hackathons & Achievements</label>
                    <textarea
                ...
                <div className="mt-6 flex flex-wrap gap-4 pt-2">
                  <button
                    ...
                    disabled={loading}
                    className="rounded-xl bg-[#22d3ee] text-[#07080d] px-6 py-3 font-bold hover:scale-[1.02] transition-all disabled:bg-slate-700 disabled:text-slate-400 flex items-center gap-2 cursor-pointer text-xs shadow-md"
                  >
                    {loading ? "Generating..." : "✨ Generate AI Summary"}
                  </button>
                  <button
                    onClick={handlePrint}
                    className="rounded-xl border border-[#22d3ee]/30 text-[#22d3ee] px-6 py-3 font-bold hover:bg-[#22d3ee] hover:text-[#07080d] hover:border-transparent transition-all flex items-center gap-2 cursor-pointer text-xs"
                  >
                    🖨️ Print / Save PDF
                  </button>
                </div>
              ...
              {/* AI Bullet Tuner Card */}
              <div className="glass-panel border-purple-500/10 rounded-3xl p-6 sm:p-8 space-y-4 shadow-lg shadow-purple-950/5">
                <h2 className="text-xl font-bold text-[#a855f7] font-display mb-1 flex items-center gap-2">
                  <span>⚡</span> AI Bullet Point Tuner
                </h2>
                <p className="text-xs text-[#94a3b8] font-medium">
                  Transform weak bullets into high-impact, ATS-optimized descriptions using the Action Verb + Result + Context formula.
                </p>

                <div className="flex flex-col gap-4 text-xs font-semibold">
                  <div className="flex flex-col gap-1">
                    <label className="text-slate-300">Weak Draft Bullet</label>
                    <textarea
                      rows={2}
                      placeholder="e.g. helped code the database logins"
                      value={draftBullet}
                      onChange={(e) => setDraftBullet(e.target.value)}
                      className="w-full rounded-xl border border-white/10 bg-white/5 p-3 text-slate-200 outline-none focus:border-purple-400 transition-all resize-none font-medium leading-relaxed"
                    />
                  </div>
                  ...
                  <button
                    onClick={tuneBullet}
                    disabled={tuning}
                    className="w-full py-2.5 bg-gradient-to-r from-[#a855f7] to-[#6366f1] hover:shadow-[0_0_15px_rgba(168,85,247,0.3)] text-white font-bold rounded-xl transition-all duration-300 disabled:bg-slate-700 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
                  >
                    {tuning ? "Tuning..." : "✨ Optimize Bullet Point"}
                  </button>

                  {improvedBullet && (
                    <div className="mt-2 rounded-2xl bg-black/45 border border-purple-500/15 p-4 text-xs text-slate-300 space-y-2 animate-fade-in-up">
                      <div className="flex justify-between items-center border-b border-purple-500/10 pb-2">
                        <span className="font-bold text-[#a855f7] uppercase tracking-wider text-[10px]">Optimized Result:</span>
                        <button
                          onClick={() => {
                      ...
                      <p className="leading-relaxed whitespace-pre-line font-medium text-slate-200">{improvedBullet}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Preview Panel */}
            <div className="resume-print-area bg-white text-black p-8 font-serif leading-relaxed shadow-2xl border border-gray-200 w-full max-w-[800px] mx-auto text-[12px] rounded-3xl min-h-[900px]">
              ...
              <div className="text-center">
                ...
                <div className="mt-1.5 flex flex-wrap justify-center items-center gap-1.5 text-[11px] text-gray-700 font-sans font-medium">
              ...
              {hasContent(summary) && (
                <div className="mt-4">
                  ...
                  <p className="mt-1.5 text-justify text-gray-800 text-xs">
                    {summary}
                  </p>
                </div>
              )}
              ...
              {hasContent(skills) && (
                <div className="mt-4">
                  ...
                  <div className="mt-1.5 space-y-0.5 text-gray-800 text-xs">
                    {skills.split("\n").map((line, idx) => {
              ...
              {hasContent(education) && (
                <div className="mt-4">
                  ...
                  <div className="mt-1.5 space-y-1 text-gray-800 text-xs font-medium">
                    {education.split("\n").map((eduLine, idx) => {
              ...
              {(hasContent(projects) || hasContent(projectBullets)) && (
                <div className="mt-4">
                  ...
                  <div className="mt-1.5 space-y-3 text-gray-800 text-xs">
              ...
              {hasContent(certifications) && (
                <div className="mt-4">
                  ...
                  <div className="mt-1.5 text-gray-800 text-xs">
          ...
          {error && (
            <div className="mt-6 p-4 rounded-3xl border border-red-500/20 bg-[#1f0d11] text-red-200 flex items-start gap-3 shadow-[0_0_20px_rgba(239,68,68,0.15)] backdrop-blur-md print:hidden text-xs">
              <span className="text-xl">⚠️</span>
              <div className="flex-1 font-semibold">
                <h3 className="font-bold text-red-400 text-sm">Generation Error</h3>
